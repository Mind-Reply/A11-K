(() => {
  ...
  function renderProjects() {
    ...
    target.innerHTML = projects.map((project) => `<article class="project-row" data-project-id="${escape(project.id)}"><div><strong>${escape(project.name)}</strong><span>${escape(project.template)} · ${escape(project.status)}</span><span class="badge-row"><em class="badge">claim: ${escape(project.claimStatus || 'needs-review')}</em><em class="badge">evidence: ${escape(project.evidenceStatus || 'none')}</em><em class="badge">approval: ${escape(project.approvalStatus || 'draft')}</em><em class="badge">provider: ${escape(project.provider || 'unassigned')}</em><em class="badge">live: no</em></span></div><div class="project-buttons"><button type="button" class="btn xs" data-project-action="generate">Generate</button><button type="button" class="btn xs ghost" data-project-action="duplicate">Duplicate</button><button type="button" class="btn xs ghost" data-project-action="publish">Package</button></div></article>`).join('');
